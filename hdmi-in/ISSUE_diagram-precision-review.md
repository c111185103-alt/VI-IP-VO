...
- TSPEC-A 第一次把系統的時序數字攤開來看，但這些數字要分兩層看待，不是全部同等可信：
  - **理論值（理想情況推算，非量測）**：`T_line=32µs`、`T_active=25.6µs`、`T_burst=240ns`、每行剛好整除成 15 個 burst。輸入參數是真的（`.bd.tcl` 裡 `PCW_ACT_FPGA1_PERIPHERAL_FREQMHZ=133.333344` 這個實際合成後的數字，不是隨口說的「約100MHz」），但套的是 zero-overhead 假設——沒有算入 `axi_mem_intercon_HP0` 對 S2MM/MM2S 兩個 master 的仲裁延遲、也沒有算 DDR 控制器本身的延遲，所以這是**理論下限**，真實硬體量到的數字只會更大，不會更小。
  - **示意值（非理論推算、非量測）**：圖上 VI/VO 之間 16µs 的相位差，純粹是為了畫圖方便挑的數字。這個不是「還沒算出來」，是真實硬體上兩個時序產生器各自獨立自由運行，相位在物理上本來就不確定，不存在一個「理論上應該是多少」的答案。
  
  `HDMI_DATAFLOW.md` 從頭到尾沒有出現過任何一個時序數字，都是文字描述「快」「慢」「同時」，TSPEC-A 至少把「理論下限大概是多少量級」攤開來了，但還沒到能驗證真實時序有沒有壓線的等級（那需要論文說的 Loop 2，實際上板用 ILA 量測）。
